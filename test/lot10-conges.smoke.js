...
(async function () {
  ...
  assert(formats.join(' | ') === 'Des journées | ½ journée | Durée libre',
    ...
    formats.join(' | ') + ')');
  assert(bornesChoisies().length >= 1, 'P1 : les deux bornes sont posées au calendrier');
  ...
  /* EXIGENCE CHANGÉE — « on revient au choix des dates » : l'écran de pose est
     ...
     feuille vide. */
  assert(bornesChoisies().length >= 1,
    'P5 : on revient au choix des dates — les bornes sont toujours au calendrier');
