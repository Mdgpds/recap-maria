...
(function (global) {
  ...
  function afficher(ctx) {
    /* LOT 32 §4 — l'en-tête du socle. Le contenu de l'écran est celui d'hier. */
    global.App.barreSlim(ctx.barre, 'Sur une période');
  ...
  function rendreFormulaire(corps) {
    ...
    corps.appendChild(Kit.ttl('Sur quel contrat'));
    var options = [['', 'Tous les contrats']].concat(etat.contrats.map(function (c) {
      return [c.id, c.prenom_enfant + (c.archive ? ' (ancien)' : '')];
    }));
    /* LOT 32 §4 — un choix cochable par contrat (`.ch`), jamais une liste
       déroulante (A.0). « Tous les contrats » reste le premier choix, et le
       choix par défaut. */
    var selContrat = choixContrat(corps, options);

    corps.appendChild(Kit.ttl('Raccourcis'));
    ...
    corps.appendChild(Kit.ttl('Ou choisir deux dates'));
  ...
  /* Les choix cochables du contrat : rend { valeur() }, l'identifiant du
     contrat choisi, ou '' pour « Tous les contrats ». */
  function choixContrat(corps, options) {
    var courante = '';
    var boutons = {};
    options.forEach(function (o) {
      var ch = Kit.bouton('ch' + (o[0] === courante ? ' on' : ''), function () {
        courante = o[0];
        Object.keys(boutons).forEach(function (k) {
          boutons[k].classList.toggle('on', k === courante);
          boutons[k].setAttribute('aria-checked', k === courante ? 'true' : 'false');
          boutons[k].querySelector('.rd').textContent = k === courante ? '✓' : '';
        });
      });
      ch.setAttribute('role', 'radio');
      ch.setAttribute('aria-checked', o[0] === courante ? 'true' : 'false');
      ch.appendChild(Kit.ce('span', 'rd', o[0] === courante ? '✓' : ''));
      var gro = Kit.ce('span', 'gro');
      gro.appendChild(Kit.ce('span', 'nm', o[1]));
      ch.appendChild(gro);
      boutons[o[0]] = ch;
      corps.appendChild(ch);
    });
    return { valeur: function () { return courante; } };
  }
  ...
  function lancer(selContrat, p) {
    ...
    if (nbMois > MAX_MOIS_PERIODE) {
      cible.appendChild(Kit.enc('w', 'Période trop longue',
    ...
    pContrats.then(function (contrats) {
      ...
      Kit.vider(cible);
      cible.appendChild(Kit.enc('w', 'Calcul impossible', Kit.messageErreur(e)));
    });
  }
  ...
  function rendreResultats(cible, p, resultats, ramenee) {
    cible.appendChild(Kit.ttl(p.titre));
    if (ramenee) {
    ...
    resultats.filter(function (r) { return r.vide || r.agr.nbMois === 0; }).forEach(function (r) {
      cible.appendChild(Kit.enc('w', r.contrat.prenom_enfant + ' n’était pas sous contrat sur cette période',
    ...
    cible.appendChild(Kit.enc('w', 'Pourquoi cette séparation',
  ...
  function vueEnsemble(utiles) {
    ...
    var pane = Kit.carte('Vue d’ensemble — ' + t.nbContrats + ' contrats');
    var l = pane;
    Kit.ligneLn(l, 'Jours de présence', Kit.jours(t.joursPresence));
    Kit.ligneLn(l, 'Indemnités d’entretien', Kit.eur(t.entretienCentimes));
    Kit.ligneLn(l, 'Heures sup acquises', Kit.heures(t.minutesSupAcquises));
    Kit.ligneLn(l, 'Congés décomptés', Kit.jours(t.joursCongesDecomptes));
    if (t.retenueSansSoldeCentimes > 0) {
      Kit.ligneLn(l, 'Retenues sans solde', '−' + Kit.eur(t.retenueSansSoldeCentimes), { alerte: true });
    }
    Kit.ligneLn(l, 'Salaires nets', Kit.eur(t.salaireNetCentimes));
    ...
    if (t.familiarisationNetCentimes || t.familiarisationEntretienCentimes) {
      Kit.ligneLn(l, 'Familiarisation — heures déclarées', Kit.eur(t.familiarisationNetCentimes));
      Kit.ligneLn(l, 'Familiarisation — entretien', Kit.eur(t.familiarisationEntretienCentimes));
    }
    Kit.ligneLn(l, 'Total versé', Kit.eur(t.totalAVerserCentimes), { total: true });
  ...
  function blocContrat(p, r) {
    ...
    if (r.tronquee) {
      bloc.appendChild(Kit.enc('w', 'Historique trop long',
    ...
    if (r.moisDemandes && a.nbMois < r.moisDemandes) {
      bloc.appendChild(Kit.enc('w', 'Ce contrat ne couvre pas toute la période demandée',
    ...
    if (r.partielsClos && r.partielsClos.length) {
      bloc.appendChild(Kit.enc('w', 'Mois clôturé compté en entier',
    ...
    var p1 = Kit.carte('Compté au jour près');
    var l1 = p1;
    Kit.ligneLn(l1, 'Jours de présence', Kit.jours(a.joursPresence));
    Kit.ligneLn(l1, 'Indemnité d’entretien', Kit.eur(a.entretienCentimes));
    if (a.familiarisationEntretienCentimes) {
      Kit.ligneLn(l1, 'Familiarisation — entretien', Kit.eur(a.familiarisationEntretienCentimes));
    }
    Kit.ligneLn(l1, 'Heures supplémentaires', Kit.heures(a.minutesSupAcquises));
    Kit.ligneLn(l1, 'Congés posés', Kit.jours(a.joursCongesDecomptes));
    if (a.retenueSansSoldeCentimes > 0) {
      Kit.ligneLn(l1, 'Retenues sans solde', '−' + Kit.eur(a.retenueSansSoldeCentimes), { alerte: true });
    }
    bloc.appendChild(p1);

    var p2 = Kit.carte('Sur les mois entiers');
    var l2 = p2;
    if (ae.nbMois === 0) {
      Kit.ligneLn(l2, 'Aucun mois entier dans cette période', '—', { discret: true });
    } else {
      Kit.ligneLn(l2, 'Mois entiers', String(ae.nbMois));
      Kit.ligneLn(l2, 'Salaires nets', Kit.eur(ae.salaireNetCentimes));
      ...
      if (ae.familiarisationNetCentimes || ae.familiarisationEntretienCentimes) {
        Kit.ligneLn(l2, 'Familiarisation — heures déclarées', Kit.eur(ae.familiarisationNetCentimes));
        Kit.ligneLn(l2, 'Familiarisation — entretien', Kit.eur(ae.familiarisationEntretienCentimes));
      }
      Kit.ligneLn(l2, 'Congés payés acquis',
        Kit.joursCp(ae.minutesCpAcquis, ae.minutesParJourConge));
      Kit.ligneLn(l2, 'Total versé sur ces mois', Kit.eur(ae.totalAVerserCentimes), { total: true });
    }
    if (r.partiels.length) {
      Kit.ligneLn(l2, 'Hors mois complets',
    ...
    if (r.soldes && r.soldes.sortie) {
      var p3 = Kit.carte('Compteurs (jamais additionnés)');
      var l3 = p3;
      var e0 = r.soldes.entree || { minutesSup: 0, minutesCpAcquis: 0, minutesCpPris: 0 };
      var s = r.soldes.sortie;
      Kit.ligneLn(l3, 'Récupération au début', Kit.heures(e0.minutesSup || 0), { discret: true });
      Kit.ligneLn(l3, 'Récupération à la fin', Kit.heures(s.minutesSup || 0));
      Kit.ligneLn(l3, 'Congés payés au début',
        Kit.joursCp(Kit.cpDisponible(e0), ae.minutesParJourConge), { discret: true });
      Kit.ligneLn(l3, 'Congés payés à la fin',
    ...
    if (a.moisProvisoires.length) {
      bloc.appendChild(Kit.enc('w', 'Mois pas encore clôturés',
    ...
    if (a.moisAvantInitialisation.length) {
      bloc.appendChild(Kit.enc('w', 'Mois antérieurs à la reprise de vos compteurs',
    ...
    if (a.moisHorsContratFiges && a.moisHorsContratFiges.length) {
      bloc.appendChild(Kit.enc('w', 'Attention — mois clôturé hors des dates du contrat',
